"""Game engine — pure state and logic, no rendering."""
...
class GameEngine:
    """Manages all game state and logic. No screen or drawing."""
    ...
    # -- Input handling -----------------------------------------------------
    def handle_keydown(self, key, unicode_char, now):
        """Process a single KEYDOWN event. Returns 'quit' to signal exit."""
        if self.state == "title":
            ...
            elif len(self.name_input) < 10 and unicode_char.isprintable() and unicode_char.strip():
                self.name_input += unicode_char
    ...
    def update(self, dt, now, keys):
        """Advance game state by one frame. Only runs during 'playing' state."""
        ...
        for inv in self.invaders.sprites():
